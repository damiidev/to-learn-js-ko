...
const timer = () => {
    if (startTime > 0){ // 10초~1초 구간일 때 카운트 다운 
        startTime -=1;
        clockText.innerText=`00:${startTime< 10? `0${startTime}` :startTime}`
    }      
    else {  // 0초일 때 
        if (score > 1){ // stage score 가 1점 초과일 때 
            alert("10초 경과 : 1점 감점💥");  
            score -=1; 
...
const reset = () => {
 ...
 const checkAnswer = (event) => {
    ...
    if(inputValue === answer[stage-1]){ // 정답 판단 
        reset();
        alert(`정답 확인 : 정답입니다!! ${realScore}점 획득!!😊`);
    }
    else{
        if(score > 0 ){   //정답이 아니고, stage 점수가 1점 초과 일 경우 
            score -=1;
            realScore = score; 
            saveStageScore(realScore); 
            paintScore();
        ...
        else{    //정답이 아니고, stage 점수가 1점 초과 일 경우 
